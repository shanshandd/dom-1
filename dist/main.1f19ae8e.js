...
var p = dom.create(' <p><span>111111</span></p>');
console.log(p);
dom.after(l3, p);
dom.before(l3, p);
var sp = dom.create('<span>test append</span>');
dom.append(box, sp);
dom.wrap(sp, '<div id="wrap"></div>');
dom.remove(p);
var arr = dom.empty(list);
console.dir(arr);
...
var span = dom.create('<span>test</span>');
dom.append(l3, span);
dom.each(dom.find('li'), function (node) {
